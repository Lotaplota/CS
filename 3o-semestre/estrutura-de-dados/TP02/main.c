...
/* Teste da função CalcularDistanciaTotal(C): Como existem 12 comandos unitários, 
o resultado esperado é 12. */ 
 
    printf("Distancia total percorrida: %d\n", CalcularDistanciaTotal(C)); 
 
/* Teste da função CalcularDistanciaGeometrica(C): no caso, a distância euclidiana 
entre (2,2) e (9,7) é sqrt((9 - 2)^2 + (7 - 2)^2) = 8.60 */ 
 
    printf("Distancia geometrica: %.2f\n", CalcularDistanciaGeometrica(C)); 
 
/* Teste da função CalcularDistanciaManhattan(C): no caso, o valor esperado é |9 - 
2| + |7 - 2| = 7 + 5 = 12 */ 
 
    printf("Distancia de Manhattan: %d\n", CalcularDistanciaManhattan(C)); 
 
/* Teste da função ContarInstrucoes(C): A sequência LLLLLLLNNNNN possui 12 
comandos. */ 
 
    printf("Quantidade de instrucoes: %d\n", ContarInstrucoes(C)); 
 
/* Teste da função DestruirCaminho(C): o resultado esperado envolve a liberação de 
toda a memória alocada para o caminho, posições e comandos*/ 
